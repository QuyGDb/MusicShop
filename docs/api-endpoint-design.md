...
| `DUPLICATE_REVIEW` | 409 | Đã review sản phẩm này trong đơn hàng này |
...
**Query params:** `page`, `limit`, `sort` (`newest`, `rating_asc`, `rating_desc`)
  ...
  "data": [
    {
      "id": "uuid",
      "rating": 5,
      "comment": "Chất lượng ép tuyệt vời",
      "createdAt": "2024-02-01T10:00:00Z",
      "user": { "id": "uuid", "name": "Nguyen Van A" }
    }
  ],
  "meta": { "pageNumber": 1, "pageSize": 20, "maxPage": 1, "totalCount": 12 }
}
```
...
Tạo review cho sản phẩm trong đơn hàng. Chỉ được tạo khi `order.status = completed`. Mỗi sản phẩm trong 1 đơn chỉ review 1 lần.
...
```json
{
  "productId": "uuid",
  "rating": 5,
  "comment": "Chất lượng tuyệt vời, âm thanh ấm"
}
```

**Response `201`:** Trả về review vừa tạo.
...
| 42 | GET | `/products/:id/reviews` | Public |
...
| 61 | POST | `/orders/:id/reviews` | Customer |
