...
Cập nhật thông tin nghệ sĩ. Sử dụng {id:guid} để xác định resource chính xác.
...
Xóa nghệ sĩ (yêu cầu {id:guid}). Chỉ xóa được khi không có release nào liên kết.
...
Cập nhật thông tin thể loại (dùng {id:guid}).
...
Xóa thể loại (dùng {id:guid}). Chỉ thực hiện khi không có Artist hoặc Release nào liên kết.
...
Cập nhật hãng đĩa (dùng {id:guid}).
...
Xóa hãng đĩa (dùng {id:guid}). Chỉ thực hiện khi không có release version nào liên kết.
...
Tạo Stripe Checkout Session cho đơn hàng đang ở trạng thái `pending`. Trả về URL để redirect người dùng.
...
Stripe Webhook — server-to-server notification. Verify signature `Stripe-Signature` và cập nhật trạng thái đơn hàng (`checkout.session.completed`).

**Auth:** Public (Verify bằng Stripe Webhook Secret)
