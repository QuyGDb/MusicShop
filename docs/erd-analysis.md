...
bio       | text     nullable    -- tiểu sử dài, dùng text thay string
country   | string   nullable    -- "UK", "US" — dùng để lọc sản phẩm
...
catalog_number   | string   nullable    -- "SHVL 804" — mã của hãng đĩa
...
note             | text    nullable   -- ghi chú của khách hàng khi đặt
...
- `shipping_address`, `shipping_name`, `shipping_phone` là snapshot — copy từ profile user lúc đặt hàng. Nếu sau này user đổi địa chỉ, đơn hàng cũ không bị ảnh hưởng.
...
- `transaction_id` là PaymentIntent ID hoặc Session ID từ Stripe.
- `paid_at` là thời điểm webhook nhận được thông báo Stripe success.
- `status = pending` cho đến khi Stripe xử lý xong thanh toán và webhook đẩy về.
...
## Mục 7 — Thông báo & Sự kiện nghiệp vụ
...
reference_id | uuid               -- id của order liên quan
...
- `type` mapping với các sự kiện trong BRD: tạo đơn, xác nhận đơn, ship đơn, hoàn thành đơn, hủy đơn.
- `reference_id` liên kết với entity liên quan — thường là `order_id`. Không tạo FK cứng vì `reference_id` trỏ đến nhiều bảng khác nhau tùy `type`.
...
| 21 | `notification_logs` | 6 | Lịch sử thông báo |
