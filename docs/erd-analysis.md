...
Khách hàng đánh giá sau khi đơn hoàn thành. Một đơn hàng có thể review nhiều sản phẩm (mỗi sản phẩm 1 review).
...
created_at | timestamp
─────────────────────────────────────────
UNIQUE (user_id, order_id, product_id)   -- mỗi sản phẩm trong 1 đơn chỉ review 1 lần
```

**Giải thích:**

- Gắn với `order_id` để enforce nghiệp vụ: chỉ review được khi `order.status = completed`. Không thể review sản phẩm chưa mua.
- `UNIQUE (user_id, order_id, product_id)` ngăn review trùng lặp — mỗi sản phẩm trong mỗi đơn chỉ được đánh giá 1 lần.
- Gắn với `product_id` (không phải `product_variant_id`) vì rating hiển thị ở cấp độ product, không cần phân biệt biến thể.
...
| 21 | `reviews` | 4 | Đánh giá sản phẩm |
...
erDiagram
  ...
  orders ||--o{ reviews : "sau hoàn thành"
...
| Chỉ review sau khi đơn Completed | Service check `order.status = completed` trước khi tạo review |
